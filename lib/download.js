...
function getChapterDatePublished(rawChapterDoc) {
  try{
    ...
    return getChapterDateGeneric(rawChapterDoc); // Twig pages (by my indexing) don't have either meta tag. We can grab the date from the time tag so that we at least have something.
  }
}
function getChapterDateGeneric(rawChapterDoc){
  return rawChapterDoc.querySelector(`time`).dateTime; // Grab the first date on the page
}
...
function getChapterDateModified(rawChapterDoc) {
  try{
    ...
    return getChapterDateGeneric(rawChapterDoc);
  }
}
