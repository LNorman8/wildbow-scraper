...
function getBodyXML(chapter, bookTitle, chapterSubstitutions, contentEl) {
  ...
  if (bookTitle.toLowerCase() == "twig") {
    lastP.remove(); // remove navigation <p> for twig. It's always the last <p>, and my attempts to detect is as garbage failed.
  }
...
function cleanSceneBreaks(xml) {
  ...
  xml = xml.replace(/<p style="text-align:center;">☙<\/p>/vg, `<p style="text-align: center;">■</p>`);
